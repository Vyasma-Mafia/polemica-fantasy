...
class TelegramSupportUpdateService(
    ...
    companion object {
        ...
        val START_REPLY: String =
            """
            Polemica Fantasy — это мини-приложение в Telegram: откройте его через кнопку меню у бота или по ссылке «Открыть» из чата с ботом.

            Чтобы обратиться в поддержку, напишите сообщение ниже боту в этом чате (чат с ботом, не в личку другим людям) — оно попадёт команде. Ответ придёт сюда же от бота.
            """.trimIndent()
    }
}
