...
interface CardPackRepository : JpaRepository<CardPack, Long> {
    ...
    @Query(
        """
        SELECT DISTINCT p FROM CardPack p
        JOIN FETCH p.rarityConfigs
        LEFT JOIN FETCH p.cardSkin
        WHERE p.active = TRUE
          AND p.autoGenerated = TRUE
        ORDER BY p.id ASC
        """,
    )
    fun findAllActiveAutoGeneratedWithRarityConfigs(): List<CardPack>
}
