...
class AchievementStage3AdminIntegrationTest {
    ...
    fun `PATCH validates system fields are not accepted and reward payloads are strict`() {
        ...
        patchAchievementWithRewards(auth, "team_submit_15", """[{"type":"PROFILE_FRAME","amount":null,"code":"","metadata":null,"displayOrder":10}]""")
            .andExpect(status().isBadRequest)
        patchAchievementWithRewards(auth, "team_submit_15", """[{"type":"COSMETIC_UNLOCK","amount":null,"code":"unknown_profile_cosmetic","metadata":null,"displayOrder":10}]""")
            .andExpect(status().isBadRequest)
        ...
        patchAchievementWithRewards(auth, "team_submit_15", """[{"type":"COSMETIC_UNLOCK","amount":null,"code":"series_winner_title","metadata":null,"displayOrder":10}]""")
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.rewards[0].type").value("COSMETIC_UNLOCK"))
