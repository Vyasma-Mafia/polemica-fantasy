...
Минимальная цена листинга = **стоимость продления** для редкости карты
(ключи `renewal.cost.*` из `economy_config`):

| Редкость | Минимальная цена | economy_config key |
|----------|------------------|--------------------|
| COMMON | 30 | `renewal.cost.COMMON` |
| RARE | 60 | `renewal.cost.RARE` |
| EPIC | 120 | `renewal.cost.EPIC` |
| LEGENDARY | 250 | `renewal.cost.LEGENDARY` |

**Обоснование:** если карту можно купить дешевле продления, продление теряет смысл.
Минимальная цена на уровне продления создаёт ценовой пол и предотвращает
перелив фантиков через символические сделки.
...
Минимальная цена задаётся существующими ключами `renewal.cost.*`.
Если в будущем потребуется независимая минимальная цена — добавим
отдельные ключи `marketplace.min_price.*`.
