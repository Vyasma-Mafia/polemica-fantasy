...
- **Справка `/help`:** в блоке «Экономика» подраздел «Маркетплейс» — комиссия %, минимум открытых паков до покупки (`minPackOpensBeforeMarketplacePurchase`), таблица мин/макс цены листинга по редкости из `economy-info` (`renewalCosts`, `marketplaceMaxPrices`).
