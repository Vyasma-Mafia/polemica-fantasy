...
- [x] Agent runtime: закрытые Fantasy/Research/Compute/Memory MCP tools, trusted immutable Research snapshots, bounded audited compute через отдельный networkless worker, sealed-decision ACT authorization, single-attempt durable intents/reconciliation, SQLite WAL/blob memory, prompt/version audit, flock и fail-closed runner.
...
- [x] Controlled Compute MCP commit `4cd95de` installed on `51.250.97.185`: descriptive stats, correlations and seeded Monte Carlo use sealed same-run Research evidence through a credential-free gateway and separate networkless worker; local/remote tests, live registry/contract/isolation/schema checks pass, writes/timer remain off.
...
- [x] Codex runtime hardened после systemd OAuth-state диагностики: отдельный writable `CODEX_HOME` с bind только существующего `auth.json` без копирования credential; strict CLI отключает shell/unified exec, rules, dependency install, subagents, web search и history; остаются exact MCP allowlists.
- [x] Manual read-only Codex runner canary `bc387600-b00d-4539-b496-f7e65d7dab6e`: `SUCCEEDED`, `hourly-compute-v1`, COMPLETE sealed evidence, один no-op decision/outcome, ноль computations/intents; независимый Fantasy read-back — 1000 фантиков, 0 карт, 0 команд.
- [x] Gameplay activation: production account/credentials and live contracts verified; staged pack/team write allowlist enabled, first free pack and MAIN team for series 273 completed with durable verified intents; hourly timer enabled. Economy/card-lifecycle/marketplace expansion remains a separate gate.
- [x] Card-aware lineup correction deployed: strategy v8 applies rarity modifiers and expected bonuses for exact card perks; two verified runs updated series 273 MAIN and BUDGET to EPIC Stereo / RARE avomarika / COMMON Macarena, with BUDGET exactly 175/175. Full runtime suite passes 166 tests; timer restored active and open intents are zero.
