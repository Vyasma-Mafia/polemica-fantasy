...
- [x] Controlled Compute MCP commit `4cd95de` pushed and staged on `codex@51.250.97.185`: descriptive stats, correlations and seeded Monte Carlo use sealed same-run Research evidence through a credential-free gateway and separate networkless worker; local/remote staging checks pass 158 tests, writes/timer remain off, privileged disabled reinstall pending.
...
- [ ] Manual read-only Codex runner canary и проверка его audit/logs: первый turn завершился без Fantasy writes, но выявил optional-null adapter errors и незаписанное решение; nullable-to-omission adapter, run-bound trusted strategy, exact decision match, обязательное решение до `SUCCEEDED`, safe exception metadata и изолированный Compute MCP исправлены/добавлены локально (158 tests), требуется redeploy и повторный canary.
