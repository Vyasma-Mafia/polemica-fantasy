...
### Key Patterns
- **Актуальное поведение `ban-pair` после 2026-05-19:** карты при санкции пары **не удаляются и не soft-delete'ятся**; result/preview по картам является информационным списком экземпляров, которые всё ещё у исходного покупателя партнёрской сделки. Повторное списание предотвращается через per-listing marker в `marketplace_listing_sanction` (`reason = "Pair ban"`, `adminUsername = "pair-ban:<low>:<high>"`): `sumSellerReceivedForSalesTo`, `findSoldFromPartnerToBuyer`, `findSoldListingsBetweenUsers` и preview-кандидаты исключают уже санкционированные SOLD-листинги. `marketplace_pair_sanction_history` остаётся аудитом запусков, но не блокирует всю пару, поэтому новые сделки после первого бана можно санкционировать отдельно.
...
- **Косметические скины карт (V40):** инстансный скин хранится на `user_card.card_skin_id` (`NULL` = дефолтный вид), источник скина при открытии — `card_pack.card_skin_id`; справочник скинов — `card_skin` (`code`, `name`). Скины не участвуют в механике очков/экономики, только в визуале. При продаже в `MarketplaceService.buyCard` сохраняется snapshot `marketplace_listing.sold_skin_code` (как и `sold_card_template_id`), чтобы feed/transaction detail оставались визуально стабильными после возможных изменений карточки у текущего владельца. Во всех user DTO карточки возвращают `skinCode`; в admin card-pack API доступны `skinId`/`skinCode` + `GET /api/v1/admin/card-skins`.
...
- **Strategy pattern для перков:** каждый детектор реализует `PerkDetector` с полем `type: String` (совпадает с `perk.id` в БД) и методом `matchCount(game, player)`; справочник — таблица `perk` (каталог после Flyway V10: `sniper`, `voteForBlack`, …)
...
├── scoring/          # DefaultScoringService, ScoringService, perk/* (detectors + registry)
...
- **`series.public_number`** — пользовательский номер бейджа «Серия N»: backend вычисляет его из последнего числа в `series.name`, а если чисел нет — `1`; поле не влияет на sync/scoring.
...
- **`card_skin` + `user_card.card_skin_id`**: скин привязан к экземпляру карты (`user_card`), а не к шаблону; это позволяет сохранять уникальный визуал карты при трейдах и переиспользовать один `card_template` для обычных и skinned экземпляров.
