#!/usr/bin/env python3
"""
Achievement balance calculator.

Takes the raw report from POST /api/v1/admin/achievement-statistics/collect
and computes balanced bonus_points so that every achievement has the same
...
def p_applicable(achievement_id: str) -> float:
    roles = ACHIEVEMENT_APPLICABLE_ROLES.get(achievement_id, [])
    return sum(ROLE_PRIORS.get(r, 0) for r in roles)


def main():
    if len(sys.argv) < 2:
        print("Usage: python balance_achievements.py <report.json> [target_E]")
        print("  report.json  — output of POST .../achievement-statistics/collect")
        print("  target_E     — desired E[bonus per game] for each achievement (default: auto = median of current)")
        sys.exit(1)

    report = json.loads(Path(sys.argv[1]).read_text())
    rows = {r["achievementId"]: r for r in report["byAchievement"]}
    ...
    for ach_id, row in sorted(rows.items()):
        ...
        results.append({
            "id": ach_id,
            "applicable_roles": ACHIEVEMENT_APPLICABLE_ROLES.get(ach_id, []),
            "p_applicable": p_app,
    ...
    print(f"{'Achievement':<25} {'P(appl)':<10} {'E[appl|appl]':<14} {'E[bonus/game]':<14} {'P(trigger/game)':<16}")
    print("-" * 80)
    ...
    print(f"{'Achievement':<25} {'Raw bp':<12} {'Rounded bp':<12} {'New E[bonus/game]':<18}")
    print("-" * 68)
    ...
    print(f"| {'Achievement':<23} | {'bonus_points':>12} | {'E[bonus/game]':>14} |")
    print(f"|{'-'*25}|{'-'*14}|{'-'*16}|")
    for r in sorted(results, key=lambda x: x["id"]):
        if r["e_bonus_per_game_bp1"] > 0:
            raw_bp = target / r["e_bonus_per_game_bp1"]
