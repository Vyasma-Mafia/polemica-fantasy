...
[[ -f "$env_file" ]] || { echo "Telegram import env file not found: $env_file" >&2; exit 1; }
[[ -d "$session_dir" ]] || { echo "Telegram import session directory not found: $session_dir" >&2; exit 1; }
...
for destination_ip in 149.154.167.51 91.108.56.130 1.1.1.1; do
  ...
  [[ "$route" == *"dev wg-tg table 201"* ]] || { echo "Route probe $destination_ip is not through wg-tg/table 201" >&2; echo "$route" >&2; exit 1; }
done
...
# Deliberately starts only the structurally shadow-only worker service.
docker compose -f "$compose_file" --profile telegram-import up -d --build --no-deps telegram-import-worker
container_id="$(docker compose -f "$compose_file" --profile telegram-import ps -q telegram-import-worker)"
stop_failed_worker() {
  docker compose -f "$compose_file" --profile telegram-import logs --tail=100 telegram-import-worker || true
  docker compose -f "$compose_file" --profile telegram-import stop telegram-import-worker || true
}
for _ in $(seq 1 30); do
  health="$(docker inspect "$container_id" --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}{{.State.Status}}{{end}}')"
  [[ "$health" == "healthy" ]] && break
  [[ "$health" == "unhealthy" || "$health" == "exited" ]] && { stop_failed_worker; exit 1; }
  sleep 5
done
[[ "${health:-}" == "healthy" ]] || { echo "Worker did not become healthy" >&2; stop_failed_worker; exit 1; }
docker compose -f "$compose_file" --profile telegram-import ps telegram-import-worker
